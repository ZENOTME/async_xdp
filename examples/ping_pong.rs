...
async fn ping_pong(dev1: &str, dev2: &str) {
    ...
    let mut dev1_context_builder = XdpContextBuilder::new(dev1, 0, runner.clone());
    dev1_context_builder
        .with_socket_config(socket_config)
        .with_exist_umem(umem.clone(), frame_manager.clone());
    let mut dev2_context_builder = XdpContextBuilder::new(dev2, 0, runner.clone());
    dev2_context_builder
        .with_socket_config(socket_config)
        .with_exist_umem(umem, frame_manager);

    let dev1_context = dev1_context_builder.build().unwrap();
    let dev2_context = dev2_context_builder.build().unwrap();

    let mut dev1_receive = dev1_context.receive_handle().unwrap();
    let mut dev2_receive = dev2_context.receive_handle().unwrap();
    let mut dev1_send = dev1_context.send_handle();
    let mut dev2_send = dev2_context.send_handle();
